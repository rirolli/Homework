...
def generic_bfs_edges(G, source, neighbors=None, depth_limit=None):
    ...
    """
    print(f"Prendo in esame il nodo sorgente \"{source}\"")
    visited = {source}
    ...
    while queue:
        ...
        try:
            child = next(children)
            print(f"Prendo in considerazione il nodo \"{child}\"") if child not in visited else print(f"Prendo in considerazione il nodo \"{child}\" ma è già stato visitato")
